...
class DeepPQK_QK(nn.Module):
    ...
    def forward(self, seq, pkt, smi):
        ...
        #smi_embed = self.smi_norm(smi_embed)
        ...
        cat = torch.cat([seq_conv*0.7, pkt_conv*1.2, smi_conv], dim=1)  # (N,640)
...
def test(model: nn.Module, test_loader, loss_function, device, show):
    ...
    targets = np.delete(targets, zero_indices)

    outputs = np.delete(outputs, zero_indices)
